    ...
    backgroundImage: `url(${isLogin ? loginBg : signupBg})`
      ...
      <div className="flex-1 bg-background rounded-t-3xl min-h-[calc(100vh-160px)] px-6 pt-6 pb-8 relative z-10 overflow-y-auto">
        <form onSubmit={handleSubmit} className="space-y-4 max-w-sm mx-auto">
          ...
          {/* Google Auth Button - Shows for both login and signup */}
          ...
          <Button type="button" variant="outline" onClick={signInWithGoogle} className="w-full h-12 text-sm border-2 border-border rounded-2xl flex items-center justify-center gap-3 hover:bg-muted transition-colors">
            <div className="w-5 h-5 bg-white rounded-sm flex items-center justify-center shadow-sm border">
            ...
            <span>{isLogin ? "Sign in with Google" : "Create account with Google"}</span>
          </Button>

          {/* Other Social Login Buttons - Only show for login */}
          {isLogin && <>
